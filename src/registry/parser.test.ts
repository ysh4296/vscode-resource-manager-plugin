...
test("parseRegistry는 정상적인 registry JSON을 파싱한다", () => {
  const registry = parseRegistry('{"resources":{"app1":{"gitlabProject":"g/app1","current":"1.0.0","versions":{}}}}');
...
test("serializeRegistry는 parseRegistry로 다시 복원 가능하고, 끝에 개행이 붙는다", () => {
  const registry: ResourceRegistry = {
    resources: { app1: { gitlabProject: "g/app1", current: "1.0.0", versions: { "1.0.0": { url: "http://x" } } } },
  };
